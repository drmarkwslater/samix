...
//			printf("* Trouve: '%s'\n",entree->d_name);
			...
			else sprintf(tempo,"%s%c%s",chemin,SLASH,entree->d_name);
...
char *LigneSuivante(char *ligne, int maxi, FILE *fichier) {
	...
	c = ligne; m = maxi - 2;
	do {
		if((*c = getc(fichier)) == EOF)  {  /* EOF: EndOfFile OU Error... */
...
//						snprintf(code,4,"%02x",*c);
//						code[2] = 0; printf("%s>",code);
//						*d++ = '@'; *d++ = code[0]; *d = code[1]; 
//						printf("%c%c%c",*(d-2),*(d-1),*d); 
//						printf("!\n");
						*d = *c;
				}
				break;
...
INLINE void ItemMinuscules(unsigned char *texte) {
	...
	c = texte; while(*c) { d = tolower(*c); *c++ = d; };
}
/* ========================================================================== */
INLINE void ItemCapitales(unsigned char *texte) {
	...
	c = texte; while(*c) {
		...
		if(isalpha(e[0]) && (*c != 0x80)) {
			if(premiere) { d = toupper(*c); premiere = 0; }
			else d = tolower(*c);
			*c++ = d;
		} else { premiere = 1; c++; }
	};
...
void ItemVarUse(ItemVar var, char *original, char *traduit, int lngr) {
	...
	ItemVar trad; int k,l,m,n; char fait;
