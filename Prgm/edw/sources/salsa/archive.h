...
static ArgDesc ArchRunInfoVoieEvt[] = {
	{ "duree(ms)",       DESC_FLOAT &VoieEcrite.def.evt.duree,       "Duree evenement (ms)" },
	...
	{ "delai(ms)",       DESC_FLOAT &VoieEcrite.def.evt.delai,       "Delai evenement (ms)" },
	{ "phase1.t0(ms)",   DESC_FLOAT &VoieEcrite.def.evt.phase[0].t0, "Debut integrale rapide (ms)" },
	{ "phase1.dt(ms)",   DESC_FLOAT &VoieEcrite.def.evt.phase[0].dt, "Duree integrale rapide (ms)" },
	{ "phase2.t0(ms)",   DESC_FLOAT &VoieEcrite.def.evt.phase[1].t0, "Debut integrale longue (ms)" },
	{ "phase2.dt(ms)",   DESC_FLOAT &VoieEcrite.def.evt.phase[1].dt, "Duree integrale longue (ms)" },
	DESC_END
};
static ArgDesc ArchRunInfoVoieTrmt[] = {
	{ "au-vol",          DESC_KEY TrmtTypeCles,   &VoieEcrite.def.trmt[TRMT_AU_VOL].type,  "Traitement au vol (neant/demodulation/lissage/filtrage)" },
//	{ "au-vol.int",      DESC_INT                 &VoieEcrite.def.trmt[TRMT_AU_VOL].p1,    "Parametre (int) du traitement au vol" },
	...
	{ "sur-tampon",      DESC_KEY TrmtTypeCles,   &VoieEcrite.def.trmt[TRMT_PRETRG].type,  "Traitement pre-trigger (neant/demodulation/lissage/filtrage)" },
//	{ "sur-tampon.int",  DESC_INT                 &VoieEcrite.def.trmt[TRMT_PRETRG].p1,    "Parametre (int) du traitement pre-trigger" },
...
static ArgDesc ArchRunInfoVoieTrgr[] = {
	...
	{ "deadline(ms)",  DESC_NONE }, // DESC_FLOAT                 &VoieEcrite.def.trgr.condition.montmax,  "Temps de montee maximum (ms)" },
...
//+ static ArgDesc ArchRunInfoVoieCoup[] = {
//+ 	DESC_END
//+ };
...
//-	{ "Type",                  DESC_KEY RunCategCles,           &RunCategNum,       "Declare au moment du lancement" },
	...
	{ "Voies.nb",              DESC_INT                     &VoiesNb,               "Nombre effectif de voies (sur toute la manip)" },
	  DESC_INCLUDE(ArchHdrEvtsNbDesc),
...
//	{ "Trigger.scrpt",         DESC_TEXT                      Trigger.nom_scrpt,    "Nom du fichier si trigger de type scrpt" },
	...
	{ "Trmt.evt.calage",       DESC_KEY CALAGES_PORTEE_CLES, &SettingsCalageMode,   "Calage des fenetres d'evenement ("CALAGES_PORTEE_CLES")" },
	...
	{ "Trmt.fen.centrage",     DESC_NONE }, // DESC_KEY CALAGES_PORTEE_CLES, &SettingsCentrageType, "Calage des fenetres ("CALAGES_PORTEE_CLES")" },
...
= {
	...
	{ "Voie.duree",             DESC_FLOAT &VoieEcrite.def.evt.duree,                "Duree evenement (ms)" },
	...
	{ "Voie.delai",             DESC_FLOAT &VoieEcrite.def.evt.delai,                "Delai evenement (ms)" },
	{ "Voie.rapide",            DESC_NONE },
	{ "Voie.phase1.t0",	        DESC_FLOAT &VoieEcrite.def.evt.phase[0].t0,          "Debut integrale rapide (ms)" },
	{ "Voie.phase1.dt",	        DESC_FLOAT &VoieEcrite.def.evt.phase[0].dt,          "Duree integrale rapide (ms)" },
	{ "Voie.phase2.t0",	        DESC_FLOAT &VoieEcrite.def.evt.phase[1].t0,          "Debut integrale longue (ms)" },
	{ "Voie.phase2.dt",	        DESC_FLOAT &VoieEcrite.def.evt.phase[1].dt,          "Duree integrale longue (ms)" },
	...
	{ "Voie.au-vol.int",        DESC_INT   &VoieEcrite.def.trmt[TRMT_AU_VOL].p1,     "1er parametre (int) du traitement au vol" },
	{ "Voie.au-vol.int2",       DESC_INT   &VoieEcrite.def.trmt[TRMT_AU_VOL].p2,     "2eme arametre (int) du traitement au vol" },
//	{ "Voie.au-vol.float",      DESC_FLOAT &VoieEcrite.def.trmt[TRMT_AU_VOL].rc,     "3eme Parametre (float) du traitement au vol" },
	...
	{ "Voie.sur-tampon.int",    DESC_INT   &VoieEcrite.def.trmt[TRMT_PRETRG].p1,     "1er parametre (int) du traitement pre-trigger" },
	{ "Voie.sur-tampon.int2",   DESC_INT   &VoieEcrite.def.trmt[TRMT_PRETRG].p2,     "2eme parametre (int) du traitement pre-trigger" },
//	{ "Voie.sur-tampon.float",  DESC_FLOAT &VoieEcrite.def.trmt[TRMT_PRETRG].rc,     "3eme parametre (float) du traitement pre-trigger" },
	...
	{ "Voie.trigger.plancher",  DESC_FLOAT &VoieEcrite.def.trgr.condition.underflow, "Amplitude (absolue) minimum avant rejet (ADU)" },
	{ "Voie.trigger.plafond",   DESC_FLOAT &VoieEcrite.def.trgr.condition.overflow,  "Amplitude (absolue) maximum avant rejet (ADU)" },
	{ "Voie.trigger.mintime",   DESC_FLOAT &VoieEcrite.def.trgr.condition.montmin,   "Temps de montee minimum avant rejet (ms)" },
	{ "Voie.trigger.deadline",  DESC_FLOAT &VoieEcrite.def.trgr.condition.montmax,   "Temps de montee maximum avant rejet (ms)" },
...
= {
	...
	{ DESC_COMMENT 0 },
	{ "Donnees.source",        DESC_NONE }, // DESC_KEY SrceCles, &SrceType,        "Source des donnees" },
	{ "Donnees.fichier",       DESC_NONE }, // DESC_TEXT          SrceName,         "Fichier de donnees si relecture (source=reel)" },
	...
	{ "Trmt.saute_evt",        DESC_BOOL                     &SettingsSauteEvt,     "Sauter evt pour calculer le pattern moyen" },
	{ "Trmt.calage",           DESC_NONE }, // DESC_BOOL  &SettingsCalageMode,        "Calage des temps" },
	{ "Trmt.evt.calage",       DESC_KEY CALAGES_PORTEE_CLES, &SettingsCalageMode,   "Calage des fenetres d'evenement ("CALAGES_PORTEE_CLES")" },
	{ "Trmt.evt.calage.mode",  DESC_KEY CALAGES_PORTEE_CLES, &SettingsCalageMode,   "Reference de calage en temps des evenement ("CALAGES_PORTEE_CLES")" },
	{ "Trmt.evt.calage.objet", DESC_KEY CALAGES_OBJET_CLES,  &SettingsCalageObj,    "Niveau de calage en temps des evenement ("CALAGES_OBJET_CLES")" },
	{ "Trmt.evt.calage.ref",   DESC_STR(MODL_NOM)            &SettingsCalageModl,   "Modele de voie de reference si calage 'unique' [auto=evt le plus court]" },
...
//	{ "Liste:63-32",         DESC_INT   &ArchEvt.liste[1],    "1 bit par detecteur touche (32..63)" },
	...
	{ "Categorie.num",       DESC_SHORT &ArchEvt.categ,       "Categorie d'evenement (numero)" },
	{ "Categorie.nom",       DESC_STR(CATG_NOM) ArchEvt.nom_catg, "Categorie d'evenement (texte)" },
	{ "Etiquette.forme",     DESC_SHORT &ArchEvt.tag.num,     "Etiquette d'affichage de la categorie" },
	{ "Etiquette.rouge",     DESC_SHORT &ArchEvt.tag.r,       "Composante rouge d'affichage de l'etiquette" },
	{ "Etiquette.vert",      DESC_SHORT &ArchEvt.tag.g,       "Composante verte d'affichage de l'etiquette" },
	{ "Etiquette.bleu",      DESC_SHORT &ArchEvt.tag.b,       "Composante bleue d'affichage de l'etiquette" },
...
= {
	...
	{ "Maxi.giga",          DESC_INT   &ArchVoie.maxigiga,             "Date du maximum (nb echantillons 100kHz modulo 1 milliard)" },
	{ "Maxi.stamp",         DESC_INT   &ArchVoie.maxistamp,            "Date du maximum (milliards d'echantillons 100kHz)" },
	...
	{ "Base",               DESC_FLOAT &ArchVoie.val[MONIT_BASE],      "Ligne de base (ADU)" },
	{ "Bruit",              DESC_FLOAT &ArchVoie.val[MONIT_BRUIT],     "Bruit (ADU)" },
//	{ "Maxi",               DESC_INT   &ArchVoie.maxi,        "Maximum (ADU)" },
	{ "Montee",             DESC_FLOAT &ArchVoie.val[MONIT_MONTEE],    "Temps de montee (millisecondes)" },
	{ "Descente",           DESC_FLOAT &ArchVoie.val[MONIT_DESCENTE],  "Temps de descente (millisecondes)" },
	{ "Amplitude",          DESC_FLOAT &ArchVoie.val[MONIT_AMPL],      "Amplitude (ADU)" },
