...
typedef enum {
	DETEC_PARM_TRMT = DETEC_PARM_ENVR,
	DETEC_PARM_REGS,
	DETEC_PARM_REGL,
	DETEC_NBDEFS
} DETEC_DEF;
...
typedef struct {
	char  nom[MODL_NOM];
	float duree;            /* duree evenement (ms)                                  */
	float delai;            /* delai evenement (ms)                                  */
	float tempsmort;        /* temps mort avant nouvelle recherche (milllisec.) (=post-trigger si <0) */
	float interv;           /* fenetre de recherche (ms)                             */
#ifdef RL
	int dimtemplate;
    float montee;
	float descente1;
...
typedef struct {
	...
	float tempsmort;        /* temps mort avant nouvelle recherche (milllisec.) (=post-trigger si <0) */
	float interv;           /* fenetre de recherche (ms)                             */
	float coinc;            /* duree possible de coincidence (ms)                    */
...
/* DETECTEUR_VAR ArgDesc ModelePhysDesc[]
 ...
 = {
	...
	{ "duree",			DESC_FLOAT &ModelePhysLu.duree,      "duree evenement (ms)" },
	{ "temps-mort",		DESC_FLOAT &ModelePhysLu.tempsmort,  "temps mort apres maxi evenement (ms)" },
	{ "fenetre",		DESC_FLOAT &ModelePhysLu.interv,     "fenetre de recherche (ms)" },
	{ "delai",			DESC_FLOAT &ModelePhysLu.delai,      "delai evenement (ms)" },
	#ifdef RL
	{ "dimtemplate",	DESC_INT   &ModelePhysLu.dimtemplate,	"dim. template (points)" },
	{ "montee",			DESC_FLOAT &ModelePhysLu.montee,		"montee (ms)" },
	{ "descente1",		DESC_FLOAT &ModelePhysLu.descente1,		"descente1 (ms)" },
	{ "descente2",		DESC_FLOAT &ModelePhysLu.descente2,		"descente2 (ms)" },
...
= {
	...
	{ "temps-mort-ms",	DESC_NONE }, // DESC_FLOAT &ModelePhysLu.tempsmort,     0 },
	{ "fenetre-ms",		DESC_NONE }, // DESC_FLOAT &ModelePhysLu.interv,        0 },
	{ "delai-ms",		DESC_NONE }, // DESC_FLOAT &ModelePhysLu.delai,         0 },
	#ifdef RL
	{ "dimtemplate",	DESC_NONE }, // DESC_INT   &ModelePhysLu.dimtemplate,   0 },
	{ "montee-ms",		DESC_NONE }, // DESC_FLOAT &ModelePhysLu.montee,	    0 },
	{ "descente1-ms",	DESC_NONE }, // DESC_FLOAT &ModelePhysLu.descente1,	    0 },
	{ "descente2-ms",	DESC_NONE }, // DESC_FLOAT &ModelePhysLu.descente2,	    0 },
	{ "facteur2",		DESC_NONE }, // DESC_FLOAT &ModelePhysLu.facteur2,	    0 },
	#endif
	DESC_END
}
#endif
...
/* DETECTEUR_VAR ArgDesc ModeleVoieDesc[]
 ...
 = {
	...
	{ "temps-mort",  DESC_FLOAT &ModeleVoieLu.tempsmort,  "temps mort apres maxi evt (ms)" },
	...
	{ "coincidence", DESC_FLOAT &ModeleVoieLu.coinc,      "duree possible de coincidence (ms)" },
	DESC_END
}
#endif
; */
...
= {
	...
	{ "temps-mort-ms",  DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.tempsmort,  0 },
	{ "pretrigger_%",   DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.pretrigger, 0 },
	{ "base.debut_%",   DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.base_dep,   0 },
	{ "base.fin_%",     DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.base_arr,   0 },
	{ "montee.debut_%", DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.ampl_10,    0 },
	{ "montee.fin_%",   DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.ampl_90,    0 },
	{ "coincidence-ms", DESC_NONE }, // DESC_FLOAT &ModeleVoieLu.coinc,      0 },
	DESC_END
}
#endif
;
...
//	byte adrs;                    /* adresse BN a utiliser                                */
	hexa ss_adrs;                 /* adresse du registre BN a utiliser                    */
	hexa negatif;                 /* adresse du registre BN a utiliser en negatif         */
	hexa identik;                 /* adresse du registre BN a utiliser de meme valeur     */
...
DETECTEUR_VAR char *VoieNom[VOIES_MAX+2];
...
#endif
;
DETECTEUR_VAR char *ArchModeDetec[ARCH_NBMODES + 2]; // special detecteur individuel, car mode "standard" en plus
...
DETECTEUR_VAR char *DetecteurAccesInitial;
...
DETECTEUR_VAR char *CapteurNom[DETEC_CAPT_MAX];
...
typedef struct {
	...
	int   dim,lngr;
#ifdef ForSamba
/* tous les cadres de cet ensemble doivent remis a zero dans OscilloVide() */
	Instrum iNb,iAcq,iDemod,iAff;
	Instrum iTemps,iAmpl,iZero,iTrig,iActif;
	Panel pTemps,pAmpl,pZero,pTrig,pDelta;
	Cadre cAmplPlus,cAmplMoins;
	Graph ecran;
    struct {
        Histo histo; HistoData hd;
		float min,max; char actif;
		Panel pAmpl; Instrum iMin,iMax; int vMin,vMax;
		Graph graph; Instrum iActif;
		Cadre cRaz,cSauve;
    } h;
#endif
	...
	float niveauX[2],niveauY[2]; int niveau,evt,moy;
} TypeOscillo,*Oscillo;
...
DETECTEUR_VAR Panel pDetecteurVoie,pDetecteurVoieSeule,pReglViAuto;
...
typedef struct {
	...
	Oscillo oscillo;
#endif
	char  complete,modifiable,compensable,validite;
	short prems,nb;
	char  mode;
	char  trig_actif;
	float trigger,deltaADU;
...
DETECTEUR_VAR int   CmdeNum,CmdeNb; DETECTEUR_VAR shex CmdeAdrs; DETECTEUR_VAR short CmdeValeur;
...
char   DetecteurVoieBrancheNumer(TypeDetecteur *detectr, short cap, int bb, short vbb, byte adrs, char *explic);
#endif
