...
int ArchRunHeader(int f, int *t0sec, int *t0msec, char log, char *explic) {
	...
	nb = ModeleVoieNb; if(nb > VOIES_MAX) nb = VOIES_MAX;
	ModeleVoie[nb].nom[0] = '\0';
#ifdef BOLOMETRES
	int vm;
...
ssize_t ArchEvtSkip(int f, char *explic) {
	...
	if(VsnManip == 1) {
		...
		ArchEvt.nbvoies = 1;
		if((rc = (ssize_t)ArgFromPath(f,ArchHdrEvt,LIMITE_ENTETE))) {
...
/* ========================================================================== */
static ssize_t ArchEvtSearch(int f, int num, int *lu, off_t *of7) {
...
/* ========================================================================== */
ssize_t ArchEvtRead(int f, int num, off_t *retour, char *explic) {
	int voie,vt,nbfiltres,i,num_lu,bolo_lu; long nb; ssize_t rc; off_t pos;
	int bolo;
	double *demarrage;
	...
	if(VsnManip == 2) {
		...
		num_lu = -1;
		if(retour) {
			if((rc = ArchEvtSearch(f,num,&num_lu,&pos)) == 0) {
				sprintf(explic,"Fin de fichier rencontree, sans marque de fin d'archivage");
				return(0);
			} else if(rc < 0) {
				if(rc == -2) *retour = -1;
