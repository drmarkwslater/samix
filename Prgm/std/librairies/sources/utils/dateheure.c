...
void DateIntToHeure(char *texte, int32 entier) {
	...
	hh = mn / 100;     mn = mn - (hh * 100);
	sprintf(texte,"%02ld:%02ld:%02ld",hh,mn,ss);
}
/* ========================================================================== */
void DateIntToJour(char *texte, int32 entier) {
	...
	aa = mm / 100;     mm = mm - (aa * 100);
	cc = aa / 100;     aa = aa - (cc * 100);
	sprintf(texte,"%02ld.%02ld.%02ld",jj,mm,aa);
}
/* ========================================================================== */
...
int32 DateJourToInt(char *texte) {
	...
	while(*c) {
		...
		if(*c == '.') {
			*c = '\0';
			// printf("(%s) element de date #%ld: %s\n",__func__,i+1,d);
			if(c > d) {
				k = 0;
				sscanf(d,"%ld",&k);
				// printf("(%s) valeur de date primaire  #%ld: %ld\n",__func__,i+1,k);
				if(!k && (i == 1)) k = ArgKeyGetIndex(mois,d) + 1;
				// printf("(%s) valeur de date definitif #%ld: %ld\n",__func__,i+1,k);
				switch(i++) {
				  case 0: jj = k; break;
				  case 1: mm = k; break;
				  case 2: aa = k; break;
	...
	if(!i) {
		if(l == 8) { sscanf(chaine,"%4ld%2ld%2ld",&aa,&mm,&jj); i = 3; }
		else { sscanf(chaine,"%2ld%2ld%2ld",&aa,&mm,&jj); i = (((int)l - 1) / 2) + 1; }
	}
	if((i < 3) && (c > d)) {
		k = 0;
		// printf("(%s) element de date #%ld: %s\n",__func__,i+1,d);
...
char *DateCode(char *texte, void *date, int *col) {
	...
	if(texte) {
		...
		DateIntToJour(DateJourTexte,*(int *)date); return(DateJourTexte);
	}
}
/* ========================================================================== */
int DateIntToJours(int32 jour) {
	int32 cc,aa,mm,jj; int an,nb;
...
int64 DateLongToSecondes(int64 date) {
	int32 jour,heure; int32 jours,secondes;

	jour = (int32)(date / 1000000); heure = (int32)(date - (jour * 1000000));
