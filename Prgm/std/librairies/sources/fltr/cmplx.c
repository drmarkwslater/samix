...
#else
double fabs(), sqrt(), atan2(), cos(), sin();
...
if( y < 1.0 )
	...
	if( (fabs(p) > w) || (fabs(q) > w) || (y == 0.0) )
		{
			//+ printf("(%s) c = (%g + i x %g) / (%g + i x %g) (|a|=%g, max=%g)",__func__,b->r,b->i,a->r,a->i,y,MAXNUM);
		c->r = MAXNUM;
		c->i = MAXNUM;
 ...
 *
 *                        2
 * Also tested by cracine( z ) = z, and tested by arguments
